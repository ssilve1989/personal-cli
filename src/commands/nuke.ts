  ...
  .action(async (target: string, opts: { force?: boolean }) => {
    ...
    if (isPort) {
      ...
      pids = result
        ...
        .map((l) => l.trim())
        .filter(Boolean)
        .map((pid) => ({ pid: parseInt(pid, 10), label: `PID ${pid} (port ${port})` }));
    } else {
      ...
      pids = result
        ...
        .map((line) => {
          ...
          return { pid: parseInt(pid, 10), label: `${pid} — ${rest.join(" ")}` };
        });
    }
    ...
    prompts.outro(`Nuked ${toKill.length} process${toKill.length > 1 ? "es" : ""}.`);
  });
